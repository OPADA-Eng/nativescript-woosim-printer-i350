...
export class WoosimPrinterI350 extends Common {
    public cPCLPrinter;
    ...
    public isConnected() {
        return this.ptrConn.getState() == 3
    }
    ...
    private sendData(data: any[]) {
        ...
        // console.log(this.ptrConn.STATE_CONNECTED);
        if (!this.isConnected()) {
            this.Toast("You are not connected !", 'short')
            if (this.debug)
    ...
    private not_english_words_in_text(text) {
    ...
    public printTextFile(file) {
        // console.log("-----printTextFile--------")
        // let inStream = application.getResources().openRawResource(file);
        this.sendData(com.woosim.printer.WoosimCmd.initPrinter());
        ...
        file.readText()
            .then((res) => {
                ...
                if (this.not_english_words_in_text(res).length > 0) {
                    ...
                    text = new java.lang.String(res).getBytes();
                }
                if (text != null) byteStream.write(text);

                byteStream.write(com.woosim.printer.WoosimCmd.printData());
                this.sendData(com.woosim.printer.WoosimCmd.initPrinter());
                this.sendData(<any>byteStream.toByteArray());
            });
    }
    // TODO: Still has a problem with bmp print ... :(
    public printImg(bitmap, setExtraPaddingAfterPrint = true, startX = 0, startY = 0) {

        try {
            let data = com.woosim.printer.WoosimImage.printBitmap(startX, startY, bitmap.getWidth(), bitmap.getHeight(), bitmap);
            if (this.debug)
                console.log("Printing Image ...");

            this.sendData(com.woosim.printer.WoosimCmd.setPageMode());
            this.sendData(com.woosim.printer.WoosimCmd.PM_setArea(1, 1, bitmap.getWidth(), bitmap.getHeight()));
            this.sendData(data);
            this.sendData(com.woosim.printer.WoosimCmd.PM_setStdMode());
    ...
    public print(string: string) {
        ...
        if (string == null)
            ...
            try {
                ...
                // }
                // else {
                    text = new java.lang.String(string).getBytes();
                // }

                let byteStream = new java.io.ByteArrayOutputStream();
                let arabic = com.woosim.printer.WoosimCmd.setCodeTable(com.woosim.printer.WoosimCmd.MCU_RX, com.woosim.printer.WoosimCmd.CP720, com.woosim.printer.WoosimCmd.FONT_SMALL);
                if (arabic != null) {
                    // console.log(arabic.length);
                    byteStream.write(arabic);
                }
                // if (text != null) byteStream.write(text);
                byteStream.write(com.woosim.printer.WoosimCmd.printData());
                this.sendData(com.woosim.printer.WoosimCmd.initPrinter());
